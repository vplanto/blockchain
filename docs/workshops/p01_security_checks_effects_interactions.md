...
## 5. Історична вразливість: `selfdestruct` та EIP-6780

Інструкція `selfdestruct` — ще один приклад того, як механізм EVM може бути використаний не за призначенням. Історично вона дозволяла **повністю видалити** код контракту з блокчейну та примусово перевести залишок ETH на будь-яку адресу (навіть на контракт без `receive`/`fallback`).

**Як це експлуатувалось:**
- Атакуючий міг примусово надіслати ETH на контракт-жертву, обійшовши будь-які перевірки `require` чи `fallback`. Це порушувало інваріанти контрактів, які покладалися на `address(this).balance` як на достовірне джерело даних.
- Контракт міг бути знищений "власником" після збору коштів від користувачів (rug pull).

> [!WARNING]
> **Актуальний стан (EIP-6780, хардфорк Cancun):** Починаючи з 2024 року, інструкція `selfdestruct` є **застарілою (deprecated)**. Вона більше **не видаляє** код чи стейт контракту. Єдина її функція тепер — примусовий переказ залишку ETH. Повне видалення контракту відбувається лише у специфічному випадку, коли контракт було створено і знищено **в межах однієї транзакції**. Не використовуйте `selfdestruct` у нових архітектурах.
...
**П4.** Що відбувається після виклику `selfdestruct` у сучасних EVM-мережах (після EIP-6780)?

**В4.** Історично `selfdestruct` назавжди видаляла код контракту та примусово переводила залишок ETH. Після хардфорку Cancun (EIP-6780, 2024) інструкція є deprecated: вона лише примусово відправляє залишок ETH на вказану адресу, але код і стан контракту **залишаються** на місці. Повне видалення можливе лише якщо контракт був створений і знищений **в межах однієї транзакції**.

---

## 🎓 Інженерний міні-кейс (на 1 бал)

Розробник написав функцію `sellTokens(uint amount)`, яка спочатку відправляє користувачу еквівалент в ETH (`payable(msg.sender).call{value: ethAmount}("")`), а потім у наступному рядку зменшує його баланс токенів (`balances[msg.sender] -= amount`). Розробник стверджує, що контракт у безпеці, оскільки перед цими рядками стоїть перевірка `require(balances[msg.sender] >= amount)`.
**Питання:** Чи є цей контракт вразливим до якоїсь атаки? Відповідь обґрунтуйте, спираючись на патерн CEI.
