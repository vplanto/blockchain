...
1. **Аналіз сирих (raw) даних транзакції:**
   ...
   ```
2. **Параметри підпису (yParity/v, r, s):**
   У виводі ви знайдете поля `r`, `s` та `yParity` (яке замінило традиційне `v` у транзакціях типу 2 після EIP-1559). Це компоненти вашого електронного цифрового підпису:
   ...
   - **$yParity$** (раніше **$v$**, Recovery ID) — вказівник на одну з двох можливих точок над віссю $x$ еліптичної кривої. Він потрібен для оптимізації математичних розрахунків EVM при відновленні публічного ключа (а класичне $v$ також запобігало транскордонним атакам відтворення — Replay Attacks, оскільки включало `Chain ID`).
3. **Механіка перевірки (ecrecover):**
   У блокчейні транзакція спочатку не містить поля `from` (хто відправив). На рівні віртуальної машини (та консенсусу ноди) алгоритм виконує операцію **відновлення публічного ключа** на базі підпису:
   - EVM бере хеш вмісту вашої транзакції ($h$) разом із $yParity, r, s$.
...
**Концепція:** В Архітектурі децентралізованих систем (EVM) кожна інструкція (opcode) коштує грошей. Сліпе тестування неприпустиме. Розробник має розуміти, скільки газу споживає його код.

### Завдання 3: Аналіз трасування та газових витрат

У стандартному проєкті Foundry вже є два ключові файли: `src/Counter.sol` (сам смарт-контракт) та `test/Counter.t.sol` (контракт-тестувальник). Запустимо тести не просто для того, щоб побачити "Pass", а щоб проаналізувати потік виконання кожної інструкції:

1. **Запуск Trace-режиму:**
   ```bash
   forge test -vvvv
   ```
   *Прапорці `-vvvv` (verbosity) розгортають кожну транзакцію в детальний дамп опкодів.*

   **Як читати вивід трасування (Traces):**
   ```text
   ...
   Traces:
     [28783] CounterTest::test_Increment()
       ...
       └─ ← [Stop]
   ```
   - Кожен рядок дерева показує глибину виклику (Call Stack).
   - Значення у квадратних дужках (напр., `[28783]`, `[22418]`) — це кількість газу, витрачена на цей конкретний виклик або внутрішню транзакцію.
   - Ви бачите, що виклик `number()` є безкоштовним для зміни стану (це `staticcall` — тільки читання), але в межах EVM-симуляції він споживає певний мінімум газу (`424`), який не списується з балансу у реальній мережі, якщо це зовнішній виклик. Водночас `increment()` здійснює реальний запис (`SSTORE`) і споживає значно більше газу на своє виконання (`22418`).
...
2. **Профілювання витрат (Gas Profiler):**
   ...
   **Аналіз газового звіту:**
   ...
   | setNumber                        | 23784           | 41258 | 43732  | 44068 | 258     |
   ```
   - **Min/Max/Avg/Median:** Показує варіативність витрат газу. Наприклад, `setNumber` має різну вартість (від `23784` до `44068`). Це тому, що запис першого значення в пустий слот пам'яті (ініціалізація `0` -> `x`) коштує 20000 газу, а перезапис вже існуючого значення (`x` -> `y`) коштує лише 5000 газу. Foundry автоматично зібрав цю статистику завдяки Fuzzing-тесту (`testFuzz_SetNumber`), який зробив понад 250 різних викликів.
   - **Deep Dive (Математика газу):** Чому у Trace-режимі `increment()` споживав `22418` газу, а в таблиці звіту він коштує `43482`? Тому що таблиця показує *повну вартість зовнішньої транзакції*, що складається з:
     - `21000` (базова ціна будь-якої транзакції в EVM)
     - `22418` (внутрішнє виконання функції `increment()`)
     - `64` (вартість Calldata: відправка 4 байтів сигнатури функції `increment()`, $4 \times 16 = 64$)
     - $21000 + 22418 + 64 = 43482$ газу. Це об'єктивна інженерна точність!
...
contract VulnerableVaultTest is Test {
    ...
    function setUp() public {
        ...
        // ВАЖЛИВО: Щоб зловмисник міг вивести більше коштів, ніж поклав, 
        // у смарт-контракті вже має бути ліквідність (гроші інших користувачів).
        // Тому ми імітуємо наявність 100 ETH у сховищі.
...
```text
[FAIL: INVARIANT VIOLATED: balance grew after withdrawal!: 115792...639187 > 0; counterexample: args=[0, 749]] testFuzz_WithdrawInvariant(uint256,uint256)
Traces:
  [48036] VulnerableVaultTest::testFuzz_WithdrawInvariant(0, 749)
    ├─ [2637] VulnerableVault::deposit()
    ├─ [27722] VulnerableVault::withdraw(749)
    │   ├─ [0] alice::fallback{value: 749}()
    │   │   └─ ← [Stop]
    │   └─ ← [Stop]
    ├─ [872] VulnerableVault::getBalance(alice) [staticcall]
    │   └─ ← [Return] 115792089237316195423570985008687907853269984665640564039457584007913129639187 [1.157e77]
    ├─ [0] VM::assertLe(115792... [1.157e77], 0, "INVARIANT VIOLATED: balance grew after withdrawal!") [staticcall]
...
[PASS] test_RevertIfNotOwner() (gas: 14223)
Suite result: ok. 1 passed; 0 failed; 0 skipped; finished in 896.60µs
```

Студент повинен переконатися, що `vm.prank` об'єктивно підмінив `msg.sender`, а `vm.expectRevert` засвідчив коректне блокування несанкціонованого доступу (тест проходить успішно `[PASS]`, оскільки контракт надійно "відбив" атаку і повернув очікувану помилку `Unauthorized()`).
...
<details>
<summary>👉 <b>Розкрити відповідь</b></summary>
