...
## 3. Core Mechanics: `_setTokenURI` vs `baseURI`
...
Після того як ви створили свій перший NFT, час зрозуміти, як інженери оптимізують контракти для масового випуску колекцій на десятки тисяч токенів.

### 4.1. ERC-721 (OpenZeppelin) vs ERC-721A (Azuki): Батч-мінтинг

Стандартна реалізація OpenZeppelin оновлює `storage` для кожного токена окремо. Якщо ви мінтите 5 NFT — це 5 окремих операцій `SSTORE` для ownership та 5 оновлень балансу. Команда Azuki (Chiru Labs) створила **ERC-721A** — альтернативну реалізацію, де мінтинг N токенів коштує майже стільки ж газу, скільки мінтинг одного.

**Як це працює (спрощено):**

```text
Стандартний ERC-721 (OpenZeppelin):
mint(tokenId=1) → SSTORE owner[1]=Alice, SSTORE balance[Alice]++
mint(tokenId=2) → SSTORE owner[2]=Alice, SSTORE balance[Alice]++
mint(tokenId=3) → SSTORE owner[3]=Alice, SSTORE balance[Alice]++
= 3 × SSTORE ownership + 3 × SSTORE balance = 6 операцій запису

ERC-721A (Azuki):
batchMint(qty=3, to=Alice) → SSTORE owner[1]=Alice, SSTORE balance[Alice]+=3
owner[2] та owner[3] не записуються! Контракт «знає», що вони належать Alice,
бо ID послідовні, і перший записаний.
= 1 × SSTORE ownership + 1 × SSTORE balance = 2 операції запису
```

**Порівняльна таблиця:**

| Критерій | ERC-721 (OpenZeppelin) | ERC-721A (Azuki) |
| :--- | :--- | :--- |
| **Мінтинг 1 токена** | ~51,000 gas | ~52,000 gas (≈ однаково) |
| **Мінтинг 5 токенів** | ~255,000 gas | ~~55,000 gas (≈ як 1 токен!) |
| **Мінтинг 100 токенів** | ~5,100,000 gas | ~60,000 gas |
| **Перший трансфер** | Стандартний | Може бути дорожчим (Lazy init) |
| **Enumerable** | Вбудований (дорогий) | Не підтримується (заощаджує газ) |
| **Коли використовувати** | Одиночний мінтинг, стандартні потреби | Drop-колекції, масовий мінтинг |

> [!WARNING]
> **Компроміс ERC-721A:** Економія газу при мінтингу досягається за рахунок «відкладеної ініціалізації» (Lazy Initialization) ownership. Це означає, що **перший трансфер** токена з середини батчу може бути дорожчим за звичайний, бо контракт виконає запис `owner[N]` лише при першому переказі. Для drop-колекцій це прийнятний компроміс: користувач платить трохи більше при перепродажі, але значно менше при мінтингу.

### 4.2. Додаткові техніки газової оптимізації

```solidity
// ❌ Антипатерн: використання Counters.sol (deprecated у OpenZeppelin 5.x)
using Counters for Counters.Counter;
Counters.Counter private _tokenIdCounter;

// ✅ Сучасний підхід: простий uint256 лічильник
uint256 private _nextTokenId;

// ❌ Антипатерн: safeMint скрізь (додаткова перевірка + газ)
_safeMint(to, tokenId); // Перевіряє IERC721Receiver у отримувача

// ✅ Якщо ви мінтите на EOA (гаманець користувача): _mint дешевше
_mint(to, tokenId); // Без перевірки — лише для EOA!
```

**Чеклист оптимізації мінтингу:**

- [ ] Використовуйте `baseURI` замість `_setTokenURI` (якщо можливо)
- [ ] Застосуйте `ERC-721A` для колекцій з масовим мінтингом
- [ ] Замініть `Counters.sol` на простий `uint256++`
- [ ] Використовуйте `_mint()` замість `_safeMint()` для EOA
- [ ] Розгляньте деплой на L2 (Base, Arbitrum) для 10-100x здешевлення газу

---

## 5. Metadata Integrity Audit
...
### 5.2. Minting Race Conditions (MEV та Front-Running)
...
**Стратегії захисту від MEV-ботів:**
...
## 6. Outside the Bubble: NFT у не-EVM мережах
...
## 7. Steelmanning: Чому On-Chain метадані можуть бути кращими
...
**The Merge ($91.8M)** — революційна **масова механіка** (Mass Mechanic). Контракт не продавав єдиний NFT; замість цього 28,983 колектори купували «маси» (units). Чим більше мас — тим більший ваш on-chain об'єкт. Це перший приклад, де *механіка смарт-контракту* визначає вигляд мистецтва: об'єднання, поглинання, зростання.
...
> ```
> - **Rarity Algorithm:** Як детермінований алгоритм розподіляв trait'и? (Наприклад, лише 9 Alien punks із 10,000).
...
## 🎓 Інженерний міні-кейс (на 2 бали, Advanced)
...
- [ ] **Створити NFT повністю самостійно:** Пройти повний Pipeline від підготовки зображення → IPFS → JSON → контракт → деплой → перегляд на OpenSea Testnet.
