...
- Це приклад, де не зображення визначає цінність, а **логіка смарт-контракту**: мистецтво — це сам алгоритм злиття, що робить колекцію дефляційною (кожен трансфер зменшує supply).
